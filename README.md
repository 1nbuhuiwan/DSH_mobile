...
> 本仓库已配置 **CI**：当你打 `v*` 标签（如 `v1.1.0`）时，会自动**构建并签名** Release APK，并发布成 **GitHub Release**。

1. 打开仓库的 **Releases** 页：`https://github.com/1nbuhuiwan/DSH_mobile/releases`
2. 找到最新版本（如 `v1.1.0`），下载 **`app-release.apk`**（正式、已签名、体积最小）。
3. 在手机上安装该 APK 即可（Android 需允许"未知来源/外部安装"）。

> 第一次发布：在本地执行 `git tag v1.1.0 && git push origin v1.1.0`，CI 会自动构建并发布 Release。
...
采用 **语义化版本 `MAJOR.MINOR.PATCH`**（当前 `1.1.0`）。**后续有任何改进建议，请把版本号向后延续**：
